...
export class GoFormService {
  ...
  ngValidators: { [k: string]: (...args: any) => string } = {
    ...
    maxlength: (obj: ValidationErrors): string =>
      `Must not exceed ${obj.requiredLength} characters, entered ${obj.actualLength}`,
    min: (obj: ValidationErrors): string =>
  ...
  requiredValidatorExists(control: FormControl | AbstractControl): boolean {
    if (control.validator) {
      const validator: ValidationErrors = control.validator(
        {} as AbstractControl
      );
